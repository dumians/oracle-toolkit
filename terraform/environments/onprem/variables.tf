...
variable "create_bucket" {
  description = "Set to false if the GCS bucket already exists and should only be reused"
  type        = bool
  default     = true
}

variable "source_db_ips" {
  description = "IP addresses of source database instances (On-Premises)"
  type        = list(string)
  default     = []
}

variable "target_db_ips" {
  description = "IP addresses of target database instances in GCP"
  type        = list(string)
  default     = []
}

variable "onprem_zdm_ip" {
  description = "Private IP address of the On-Premises ZDM Service Node VM"
  type        = string
  default     = "10.10.1.100"
}

variable "onprem_ogg_ip" {
  description = "Private IP address of the On-Premises GoldenGate Microservices Hub VM"
  type        = string
  default     = "10.10.1.101"
}

variable "deploy_goldengate" {
  description = "Set to true if deploying GoldenGate 23ai CDC on the On-Premises Hub"
  type        = bool
  default     = false
}
